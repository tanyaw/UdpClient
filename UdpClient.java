        ...
        try (Socket socket = new Socket("codebank.xyz", 38005)) {
			...
			for (int i = 0; i < 13; i++){
				...
				if (i == 0){
					...
					// Reinitialize size (to be multiplied by 2 later on)
					size = 1;
				}
				...
				// Construct UDP packets for data
					...
					/* [12] [13] Source port already initialized to 0 */

					// Destination port [14] already 0
					pseudoheader[15] = udpHeader[2];
	
					// Length [16] already 0
					pseudoheader[17] = udpHeader[4];
					...
					pseudoheader[18] = (byte)((int)(udpChecksum >> 8) & 0xFF);
					pseudoheader[19] = (byte)((int)udpChecksum & 0xFF); 
				...
				// Otherwise, send UDP header + UDP data
				else{
					os.write(udpHeader);
					os.write(udpData);
				...
				// Get port number
				if (i == 0){
	        		// Read from the input stream and concatenate 
	        		b1 = is.read();
	        		b2 = is.read();
